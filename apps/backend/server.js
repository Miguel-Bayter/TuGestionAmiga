...
let usuarioColumnsPromise = null;
const getUsuarioColumns = async () => {
  if (usuarioColumnsPromise) return usuarioColumnsPromise;

  usuarioColumnsPromise = (async () => {
    try {
      const [rows] = await pool.query('SHOW COLUMNS FROM usuario');
      const set = new Set();
      for (const r of rows || []) {
        if (r?.Field) set.add(String(r.Field));
      }
  ...
  return usuarioColumnsPromise;
};

const resetUsuarioColumnsCache = () => {
  usuarioColumnsPromise = null;
};

const getUsuarioPasswordColumn = async () => {
  const cols = await getUsuarioColumns();
  if (cols.has('password')) return '`password`';
  if (cols.has('contraseña')) return '`contraseña`';
  return '`password`';
};
...
const FRONTEND_DIST_ROOT = path.join(FRONTEND_ROOT, 'dist');
const FRONTEND_PUBLIC_ROOT = path.join(FRONTEND_ROOT, 'public');
...
if (HAS_FRONTEND_DIST) {
  app.use(express.static(FRONTEND_DIST_ROOT, { dotfiles: 'deny' }));
  app.get(/^\/(?!api(?:\/|$)|src\/assets\/).*/, (req, res) =>
    res.sendFile(path.join(FRONTEND_DIST_ROOT, 'index.html'))
  );
} else {
